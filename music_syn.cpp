...
double sqr(double num)
{
	if(num==0)
		return 0;
	return (sin(num)/abs(sin(num))+1)/2;
} 
//合成器函数
double synth_func(double freq)
{
	if(freq==0)
		return 0;
	double res=0;
	int repeat=25;
	for(float i=1.0;i<=repeat;i+=0.5)
		res+=sqr(i*freq);
	res/=freq;
	res*=10;
	return res;
...
void MakeWaveData(int rate, int freq, int amp, char* p, int len, double synth(double)=sin)
{
	...
	if (m_channelbits == 16) //16位
	{
		if (m_channels == 1)
		{
			for (int i = 0; i < len; i++)
			{
				INT16 v = INT16(amp/100*32768 * synth(freq * i / rate));
				*(p + flag) = v & 0xFF;//低8位
...
int main(int argc,char** argv)
{
	...
	create_wave(v,100,4500.0/speed,fn.substr(0,fn.length()-4)+"_syn.wav",synth_func);
	cout<<"done";
}
